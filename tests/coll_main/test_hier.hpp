...
TEST_CASE( "Testing Hierarchy fit -- Main", "[Hie]" ) {
    ...
    for ( unsigned to = 0; to < num_files; to++ ) {
        ...
        std::string folder_nt = "./coll_main/results/" + n_tau[to];
        ...
        // Load prebuilt files and check results
        unsigned columns = ( dim_param + 1 ) * 2;
        Input inp = { file_fit_out, w_size, columns + 1, 1 };
        Matrix check = load_data( inp );

        // Check the values from the files and the genearated
        for ( unsigned i = 0; i < check.row_size; i++ ) {
            unsigned aux = 0;
            for ( unsigned j = 1; j < check.col_size; j++ ) {
                REQUIRE(
                    res_avgstd[i * columns + aux] == 
                    Approx( check.data[i *(columns + 1) + j] ) );
                aux += 1;
            }
        }

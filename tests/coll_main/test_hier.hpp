#ifndef TEST_HIER_HPP
#define TEST_HIER_HPP
...
TEST_CASE( "Testing Hierarchy fit -- Main", "[Hie]" ) {

    // Generate the results folder if it does not exists
    mkdir("coll_main/results", 0777); // Where the results are stored

    // Read data from input file
    std::string input_f = "./coll_main/input.in";
    ...
    for ( unsigned ni = 0; ni < num_files; ni++ ) {
        ...
        std::string folder_nt = "./coll_main/results/" + n_tau[ni];
    ...
    for ( unsigned ni = 0; ni < num_files; ni++ ) {
        ...
        file_est_out = "./coll_main/results/" + n_tau[ni] + \
            "/b_central_" + file_const[0] + n_tau[ni] + "x" + \
            n_space[0] + file_const[1] + "." + str_boots[0] + ".dat";
        ...
        // Test the results by loading the files just created
        unsigned columns = ( dim_param + 1 ) * 2;
        Input input = { file_fit_out, w_size, columns + 1, 1 };
        Matrix data_file = load_data( input );
        for ( unsigned nr = 0; nr < w_size; nr++ ) {
            for ( unsigned nc = 0; nc < columns; nc++ ) {
                REQUIRE(
                    data_file.data[nr * ( columns + 1 ) + nc + 1] ==
                    Approx( res_avgstd[nr * columns + nc] )
                );
            }
        }

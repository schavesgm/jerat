...
double f_1( 
    std::vector<double> params, double x, std::vector<double> ) {
...
TEST_CASE( "Testing Fit", "[Fit]" ) {
    ...
    SECTION( "Testing chi square" ) {
        ...
        for ( unsigned i = 0; i < inputs; i++ ) {
            ...
            vals_2[i * 2] = x;
            vals_3[i * 2] = x;
            vals_1[i * 2 + 1] = f_1( params, x, args );
            vals_2[i * 2 + 1] = f_1( params, x, args ) + 0.1;
            vals_3[i * 2 + 1] = f_1( params, x, args ) + 0.01;
        }
        Matrix data_1 = { vals_1, inputs, 2, 1 };
        Matrix data_2 = { vals_2, inputs, 2, 1 };
        Matrix data_3 = { vals_3, inputs, 2, 1 };

        double chi_1 = chi_square( f_1, params, data_1, args );
        double chi_2 = chi_square( f_1, params, data_2, args );
        double chi_3 = chi_square( f_1, params, data_3, args );
    ...
    SECTION( "Testing linear Nelder-Mead - Case 1" ) {
        ...
        for( unsigned i = 0; i < num_points; i++ ) {
            vals[i * 2] = 0.1 * i;
            vals[i * 2 + 1] = f_1( true_params, 0.1 * i, args ) + \
                0.01 * sampler(r_eng);
        }
    ...
    SECTION( "Testing linear Nelder-Mead - Case 2" ) {
        ...
        for( unsigned i = 0; i < num_points; i++ ) {
            vals[i * 2] = 0.1 * i;
            vals[i * 2 + 1] = f_2( true_params, 0.1 * i, args ) + \
                0.001 * sampler(r_eng);
        }
        Matrix data = { vals, num_points, 2, 1 };
    ...
    SECTION( "Testing non-linear Nelder-Mead - Case 1" ) {
        ...
        for( unsigned i = 0; i < num_points; i++ ) {
            vals[i * 2] = 0.1 * i;
            vals[i * 2 + 1] = f_3( true_params, 0.1 * i, args ) + \
                0.01 * sampler(r_eng);
        }
        Matrix data = { vals, num_points, 2, 1 };
    ...
    SECTION( "Testing non-linear Nelder-Mead - Case 2" ) {
        ...
        for( unsigned i = 0; i < num_points; i++ ) {
            vals[i * 2] = i;
            vals[i * 2 + 1] = f_4( true_params, i, args ) + \
                0.01 * sampler(r_eng);
        }
        Matrix data = { vals, num_points, 2, 1 };
